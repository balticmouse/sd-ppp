...
export default function(sdppp) {
     ...
     * 
     * If the first widget is a number type and its precision is suitable for dragging, only keep the first widget
    ...
    sdppp.widgetable.add('*rgthree*', (node) => {
        // 判断是否是分组类节点（通过名称包含"group"识别，不区分大小写）
        const isGroupNode = node.title.toLowerCase().includes('group');
        return {
            title: node.title,
